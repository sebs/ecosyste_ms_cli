...
@click.group(help="Commands for the sbom API.")
@common_options
@click.pass_context
def sbom(ctx, timeout, format, domain):
    """Commands for the sbom API."""
...
def _register_commands():
    """Register all commands for the sbom API dynamically from OpenAPI spec."""
    # Load the OpenAPI spec
    spec = load_api_spec("sbom")
        ...
        for method, operation in path_item.items():
            if method in ["get", "post", "put", "delete", "patch"]:
                ...
                if not parameters:
                    ...
                    @sbom.command(name=command_name, help=description)
                    @api_command("sbom", operation_id=operation_id)
                    def command_impl():
                        pass
                    ...
                    def create_command(op_id, cmd_name, desc):
                        @sbom.command(name=cmd_name, help=desc)
                        @api_command("sbom", operation_id=op_id)
                        def command_impl():
                            pass
