...
@click.group(help="Commands for the parser API.")
@common_options
@click.pass_context
def parser(ctx, timeout, format, domain):
    """Commands for the parser API."""
...
def _register_commands():
    """Register all commands for the parser API dynamically from OpenAPI spec."""
    # Load the OpenAPI spec
    spec = load_api_spec("parser")
        ...
        for method, operation in path_item.items():
            if method in ["get", "post", "put", "delete", "patch"]:
                ...
                if not parameters:
                    ...
                    @parser.command(name=command_name, help=description)
                    @api_command("parser", operation_id=operation_id)
                    def command_impl():
                        pass
                    ...
                    def create_command(op_id, cmd_name, desc):
                        @parser.command(name=cmd_name, help=desc)
                        @api_command("parser", operation_id=op_id)
                        def command_impl():
                            pass
