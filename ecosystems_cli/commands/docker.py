...
def _operation_id_to_command_name(operation_id: str) -> str:
    ...
    - usageEcosystem -> usage_ecosystem
    """
...
@click.group(help="Commands for the docker API.")
@common_options
@click.pass_context
def docker(ctx, timeout, format, domain):
    """Commands for the docker API."""
...
def _register_commands():
    """Register all commands for the docker API dynamically from OpenAPI spec."""
    # Load the OpenAPI spec
    spec = load_api_spec("docker")
        ...
        for method, operation in path_item.items():
            if method in ["get", "post", "put", "delete", "patch"]:
                ...
                if not parameters:
                    ...
                    @docker.command(name=command_name, help=description)
                    @api_command("docker", operation_id=operation_id)
                    def command_impl():
                        pass
                    ...
                    def make_command(op_id):
                        ...
                        def command_impl(ctx, timeout, format, domain, *args, **kwargs):
                            ...
                            execute_api_call(ctx, "docker", operation_id=op_id, call_args=args, call_kwargs=kwargs)
