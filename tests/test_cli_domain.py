...
class TestCLIDomainConfiguration:
    ...
    @patch("ecosystems_cli.commands.execution.bravado_factory")
    def test_domain_parameter_in_main_command(self, mock_bravado_factory):
        """Test --domain parameter at main command level."""
        mock_bravado_factory.call.return_value = {"advisories": []}

        self.runner.invoke(main, ["--domain", "custom.api.com", "advisories", "get_advisories"])

        # Check that the custom domain was used in the URL
        mock_bravado_factory.call.assert_called()
        call_args = mock_bravado_factory.call.call_args
        assert call_args.kwargs["base_url"] == "https://custom.api.com/api/v1"

    @patch("ecosystems_cli.commands.execution.bravado_factory")
    def test_domain_parameter_in_subcommand(self, mock_bravado_factory):
    ...
    @patch("ecosystems_cli.commands.execution.bravado_factory")
    def test_domain_env_var_general(self, mock_bravado_factory, monkeypatch):
    ...
    @patch("ecosystems_cli.commands.execution.bravado_factory")
    def test_domain_env_var_api_specific(self, mock_bravado_factory, monkeypatch):
    ...
    @patch("ecosystems_cli.commands.execution.bravado_factory")
    def test_domain_precedence_param_over_env(self, mock_bravado_factory, monkeypatch):
    ...
    @patch("ecosystems_cli.commands.execution.bravado_factory")
    def test_domain_precedence_api_env_over_general(self, mock_bravado_factory, monkeypatch):
    ...
    @patch("ecosystems_cli.commands.execution.bravado_factory")
    def test_domain_inherited_from_parent_context(self, mock_bravado_factory):
        ...
        # Verify bravado_factory.call was called with the correct base_url
        mock_bravado_factory.call.assert_called_once()
        call_args = mock_bravado_factory.call.call_args
